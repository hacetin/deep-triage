...
def load_data(dataset_name, min_train_samples_per_class):
    ...
    all_data = np.load("./data/{0}/all_data_{1}.npy".format(dataset_name, min_train_samples_per_class), allow_pickle=True)
    all_owner = np.load("./data/{0}/all_owner_{1}.npy".format(dataset_name, min_train_samples_per_class), allow_pickle=True)
...
def embedding(sentences, labels, unique_labels, wordvec_model, vocabulary, max_sentence_len = 50, embed_size_word2vec = 200):
    ...
    X = np.empty(shape=[len(sentences), max_sentence_len, embed_size_word2vec], dtype='float32')
...
def chronological_cv(num_cv, sentences, labels, wordvec_model):
    ...
    for i in range(1, num_cv+1):
        ...
        # Remove all the words that is not present in the vocabulary
        updated_train_data, updated_train_owner = filter_with_vocabulary(train_data, train_owner, vocabulary)
        final_test_data, final_test_owner = filter_with_vocabulary(test_data, test_owner, vocabulary)

        # Remove those classes from the test set, for whom the train data is not available.
        updated_test_data, updated_test_owner = filter_with_labels(final_test_data, final_test_owner, updated_train_owner)

        # Create the data matrix and labels required for the deep learning model training and softmax classifier
        unique_train_label = list(set(updated_train_owner))
        classes = np.array(unique_train_label)
        X_train, Y_train = embedding(updated_train_data, updated_train_owner, unique_train_label, wordvec_model, vocabulary)
        X_test, Y_test = embedding(updated_test_data, updated_test_owner, unique_train_label, wordvec_model, vocabulary)
...
def google_chromium_chronological_cv(min_train_samples_per_class=0, numCV=10):
    ...
    wordvec_model, all_data, all_owner = load_data("google_chromium", min_train_samples_per_class)
...
def mozilla_core_chronological_cv(min_train_samples_per_class=0, numCV=10):
    # Load preprocessed data
    wordvec_model, all_data, all_owner = load_data("mozilla_core", min_train_samples_per_class)

    return chronological_cv(10, all_data, all_owner, wordvec_model)

def mozilla_firefox_chronological_cv(min_train_samples_per_class=0, numCV=10):
    # Load preprocessed data
    wordvec_model, all_data, all_owner = load_data("mozilla_firefox", min_train_samples_per_class)
